...
class pdf {
	 ...
	 */
	public static function create($content, $pdf_command, $header = null, $footer = null) {
		...
		// Create temporary file for footer (if necessary)
		if (isset($footer)) {
			// Create temporary file for the footer
			$temp_footer = self::secure_tmpname('.html', 'footer');
			if (false === $temp_footer) {
				return false;
			}

			// Write footer to temporary file
			if (false === file_put_contents($temp_footer, $footer)) {
				return false;
			}

			$options .= sprintf(PDF_COMMAND_FOOTER_OPTION, $temp_footer);
		}
