...
foreach ($campaigns as $campaign) {
    // Possible statuses: 'save' (draft), 'paused', 'schedule', 'sending', 'sent'
    if (in_array($campaign['status'], ['save', 'schedule'])) {
        $draft_scheduled_campaigns[] = $campaign;
...
if (!empty($draft_scheduled_campaigns)) {
    ...
    foreach ($draft_scheduled_campaigns as $c) {
        ...
        // Convert times to WordPress timezone
        $create_time = new DateTime($c['create_time'], new DateTimeZone('UTC'));
        $create_time->setTimezone(new DateTimeZone(get_option('timezone_string')));

        $scheduled_time = '-';
        if (!empty($c['settings']['send_time'])) {
            $send_time = new DateTime($c['settings']['send_time'], new DateTimeZone('UTC'));
            $send_time->setTimezone(new DateTimeZone(get_option('timezone_string')));
        ...
        // Example delete URL (handle via admin.php?action=delete_campaign, etc.)
...
if (!empty($sent_campaigns)) {
    ...
    foreach ($sent_campaigns as $c) {
        // Status will always be 'sent' here
        ...
        if (!empty($c['send_time'])) {
            ...
            $st->setTimezone(new DateTimeZone(get_option('timezone_string')));
        ...
        // Pull data from report_summary if available
        $report_summary = '';
        if (!empty($c['report_summary'])) {
            // Just show a summary (unique_opens, open_rate, etc.)
