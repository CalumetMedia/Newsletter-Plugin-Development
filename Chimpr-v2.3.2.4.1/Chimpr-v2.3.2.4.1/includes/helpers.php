    ...
    function get_newsletter_posts($blocks) {
        ...
        foreach ($blocks as $block) {
            ...
            switch ($block['type']) {
                ...
                case 'wysiwyg':
                    // Always include WYSIWYG blocks, even if empty
...
if ($block_data['type'] === 'content') {
    ...
    // Ensure content is unslashed and properly formatted
    $content = wp_unslash($content);
    // Add wpautop only if content doesn't already have paragraphs and is not empty
    if (!empty($content) && strpos($content, '<p>') === false) {
        $content = wpautop($content);
    }
    // Always render the container, even if content is empty
    ...
    function newsletter_handle_blocks_form_submission_non_ajax($newsletter_slug) {
        ...
        foreach ($blocks as $block) {
            ...
            if ($block['type'] === 'wysiwyg' && isset($block['wysiwyg'])) {
                $processed_block['wysiwyg'] = wp_kses_post(wp_unslash($block['wysiwyg']));
            }

            $processed_blocks[] = $processed_block;
        }

        update_option("newsletter_blocks_$newsletter_slug", $processed_blocks);
