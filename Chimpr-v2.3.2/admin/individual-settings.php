...
// Retrieve all available PDF templates
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['settings_nonce']) || !wp_verify_nonce($_POST['settings_nonce'], 'save_settings_action')) {
        ...
        if (!empty($_POST['newsletter_is_ad_hoc'])) {
            // If ad hoc is enabled, clear schedule settings
            update_option("newsletter_send_days_$newsletter_slug", []);
            update_option("newsletter_send_time_$newsletter_slug", '');
            update_option("newsletter_send_time_utc_$newsletter_slug", '');
        }

        // Attempt to create a UTC-based schedule time for reference
        $raw_time = sanitize_text_field($_POST['newsletter_send_time']);
        if (!empty($raw_time) && empty($_POST['newsletter_is_ad_hoc'])) {
            $tz = wp_timezone();
            $now_local = new DateTime('now', $tz);
            $date_local = new DateTime($now_local->format('Y-m-d').' '.$raw_time.':00', $tz);
            if ($date_local <= $now_local) {
                // If the time already passed today, move to the next day
                $date_local->modify('+1 day');
            }

            $date_local_utc = clone $date_local;
            $date_local_utc->setTimezone(new DateTimeZone('UTC'));
            update_option("newsletter_send_time_utc_$newsletter_slug", $date_local_utc->format('c'));
        } else {
            update_option("newsletter_send_time_utc_$newsletter_slug", '');
        }
...
<div class="wrap">
    ...
    <style>
        <?php echo file_get_contents(dirname(plugin_dir_path(__FILE__)) . '/assets/css/newsletter-admin.css'); ?>
    </style>
                            ...
                            <select id="newsletter_pdf_template_id" name="newsletter_pdf_template_id" <?php echo ($opt_into_pdf ? '' : 'disabled'); ?>>
                                ...
                                if (!empty($templates) && is_array($templates)) {
                                    foreach ($templates as $index => $template_item) {
                                        ...
                                        echo '<option value="' . esc_attr($index) . '" ' . $selected . '>' . esc_html($template_name) . '</option>';
                                    }
                                } else {
