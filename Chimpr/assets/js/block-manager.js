...
window.initializeBlock = function(block) {
    try {
        ...
        // Initialize show title toggle with proper state preservation
        const showTitleToggle = block.find('.show-title-toggle');
        if (showTitleToggle.length) {
            const initialState = showTitleToggle.prop('checked');
            console.log('Initial show title state:', initialState);
            
            showTitleToggle.off('change').on('change', function() {
                if (window.newsletterState.isReady) {
                    const isChecked = $(this).prop('checked');
                    console.log('Show title toggled:', isChecked);
...
// Enhanced block type change handler
...
function setupWysiwygBlock(block, existingContent) {
    ...
    window.addBlock = function() {
        ...
        newBlock.find('.manual-override-toggle').on('change', function() {
            ...
            console.log('[Manual Override] Toggle changed:', {
                blockId: $block.attr('data-block-index'),
                isManual: isManual,
            ...
            if (window.newsletterState.isReady) {
                console.log('[Manual Override] Triggering preview update');
        ...
        function handleManualOverrideToggle($block, isManual) {
            console.log('[Manual Override Handler] Starting toggle handling:', {
                blockId: $block.attr('data-block-index'),
                isManual: isManual,
                currentState: {
                    postsList: $block.find('.sortable-posts').length,
                    storyCount: $block.find('.block-story-count').val(),
                    checkboxesCount: $block.find('input[type="checkbox"]').length
                }
            });
            ...
            console.log('[Manual Override Handler] UI updated:', {
                pointerEvents: $postsList.css('pointer-events'),
                opacity: $postsList.css('opacity'),
                storyCountDisabled: $storyCount.prop('disabled'),
                checkboxesDisabled: !isManual
            });

            // If switching to automatic mode, apply story count
            if (!isManual && $storyCount.val() !== 'disable') {
                console.log('[Manual Override Handler] Switching to automatic mode, applying story count:', $storyCount.val());
                handleStoryCountChange($block, $storyCount.val());
            }
        }
...
function initWysiwygEditor(block) {
    ...
    // Load initial content if none exists
    if (!existingContent) {
        const loadUrl = window.ajaxurl;
        $.ajax({
            url: loadUrl,
            type: 'POST',
            async: false, // Synchronous to ensure content is loaded before editor init
            data: {
    ...
    // Initialize new editor with content
    if (typeof wp !== 'undefined' && wp.editor) {
        wp.editor.initialize(editorId, {
            tinymce: {
                wpautop: true,
                ...
                setup: function(editor) {
                    ...
                    // Save content before any form submission
