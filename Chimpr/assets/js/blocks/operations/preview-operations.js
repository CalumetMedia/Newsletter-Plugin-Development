/**
 * Preview Operations
 * @module blocks/operations/preview-operations
 * @description Handles preview generation and updates
 */

import { blockStore } from '../state/store';

class PreviewOperations {
    constructor() {
        this.previewUpdatePromise = null;
        this.previewInitialized = false;
    }

    /**
     * Update preview content
     * @param {string} trigger What triggered the update
     * @returns {Promise} Preview update promise
     */
    updatePreview(trigger = 'manual') {
        // Cancel any existing preview update
        if (this.previewUpdatePromise?.abort) {
            this.previewUpdatePromise.abort();
        }

        const $previewContent = $('#preview-content');
        $previewContent.addClass('loading');

        const state = blockStore.getState();
        const blocks = this.collectPreviewData();

        // Generate preview
        this.previewUpdatePromise = $.ajax({
            url: newsletterData.ajaxUrl,
            method: 'POST',
            data: {
                action: 'generate_preview',
                security: newsletterData.nonceGeneratePreview,
                newsletter_slug: newsletterData.newsletterSlug,
                saved_selections: JSON.stringify(blocks),
                header_template: $('#header_template').val(),
                footer_template: $('#footer_template').val()
            },
            success: (response) => {
                if (response.success) {
                    $previewContent.html(response.data);
                } else {
                    console.error('Preview generation failed:', response.data);
                    $previewContent.html('<p class="error">Error generating preview</p>');
                }
            },
            error: (xhr, status, error) => {
                if (status !== 'abort') {
                    console.error('Ajax error:', error);
                    $previewContent.html('<p class="error">Error generating preview</p>');
                }
            },
            complete: () => {
                $previewContent.removeClass('loading');
                this.previewUpdatePromise = null;
            }
        });

        return this.previewUpdatePromise;
    }

    /**
     * Initialize preview functionality
     */
    initializePreview() {
        if (this.previewInitialized) return;
        this.previewInitialized = true;

        // Initial preview update
        this.updatePreview('initial_load');

        // Set up event listeners
        this.setupPreviewEventListeners();
    }

    /**
     * Set up preview event listeners
     * @private
     */
    setupPreviewEventListeners() {
        // Block changes
        $('#blocks-container').on('change', 
            '.block-type, .block-template, input[name*="[show_title]"], .block-title-input, input[type="checkbox"]', 
            () => {
                if (window.newsletterState?.isReady) {
                    if (typeof debouncedAutoSave === 'function') {
                        debouncedAutoSave();
                    }
                    this.updatePreview('block_change');
                }
            }
        );

        // Template changes
        $('#header_template, #footer_template').on('change', () => {
            if (window.newsletterState?.isReady) {
                if (typeof debouncedAutoSave === 'function') {
                    debouncedAutoSave();
                }
                this.updatePreview('template_change');
            }
        });

        // WYSIWYG changes
        $('#blocks-container').on('change keyup paste input', '.wysiwyg-editor-content', function() {
            if (window.newsletterState?.isReady) {
                const editorId = $(this).attr('id');
                if (tinymce.get(editorId)) {
                    tinymce.get(editorId).save();
                }
                this.updatePreview('wysiwyg_content_change');
            }
        });
    }

    /**
     * Collect preview data from blocks
     * @private
     * @returns {Array} Block data for preview
     */
    collectPreviewData() {
        const blocks = [];
        $('#blocks-container .block-item').each(function() {
            const $block = $(this);
            const blockData = {
                type: $block.find('.block-type').val(),
                title: $block.find('.block-title-input').val(),
                show_title: $block.find('input[name*="[show_title]"]').prop('checked'),
                template_id: $block.find('.block-template').val(),
                category: $block.find('.block-category').val(),
                date_range: $block.find('.block-date-range').val(),
                story_count: $block.find('.block-story-count').val(),
                manual_override: $block.find('input[name*="[manual_override]"]').prop('checked'),
                posts: {}
            };

            // Get selected posts
            $block.find('.sortable-posts input[type="checkbox"]:checked').each(function() {
                const $checkbox = $(this);
                const postId = $checkbox.closest('li').data('post-id');
                const order = $checkbox.closest('li').find('.post-order').val();
                blockData.posts[postId] = {
                    checked: '1',
                    order: order
                };
            });

            // Handle special block types
            if (blockData.type === 'html') {
                blockData.html = $block.find('textarea[name*="[html]"]').val();
            } else if (blockData.type === 'wysiwyg') {
                const editorId = $block.find('.wysiwyg-editor-content').attr('id');
                if (tinymce.get(editorId)) {
                    tinymce.get(editorId).save();
                    blockData.wysiwyg = tinymce.get(editorId).getContent();
                } else {
                    blockData.wysiwyg = $block.find('.wysiwyg-editor-content').val();
                }
            }

            blocks.push(blockData);
        });

        return blocks;
    }
}

export const previewOperations = new PreviewOperations(); 