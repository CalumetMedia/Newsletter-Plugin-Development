...
function newsletter_stories_handle_form_submission() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['blocks_nonce'])) {
        ...
        // Check if user clicked reset blocks
        ...
        if (isset($_POST['blocks'])) {
            ...
            // Optional debug for WYSIWYG blocks
            ...
            foreach ($blocks as $block) {
                ...
                if ($sanitized_block['type'] === 'wysiwyg') {
                    // If you have a function to sanitize WYSIWYG, call it here:
                    // $processed_block = handle_wysiwyg_content($block, null);
        ...
        // ============================
        // Save single "target_tag"
        // ============================
        error_log('[Form Handler] Checking for target_tag in POST data');
        if (isset($_POST['target_tag'])) {
            $tag_selected = sanitize_text_field($_POST['target_tag']);
            update_option("newsletter_target_tag_$newsletter_slug", $tag_selected);
            error_log('[Form Handler] Saved single tag: ' . $tag_selected);

            // Verify the save
            $verified_tag = get_option("newsletter_target_tag_$newsletter_slug", '');
            error_log('[Form Handler] Verified single tag from DB: ' . $verified_tag);
        } else {
