...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (
        !isset($_POST['mailchimp_settings_nonce']) ||
        !wp_verify_nonce($_POST['mailchimp_settings_nonce'], 'update_mailchimp_settings')
    ) {
        ...
        update_option('mailchimp_from_name', sanitize_text_field($_POST['mailchimp_from_name']));
        update_option('mailchimp_reply_to', sanitize_email($_POST['mailchimp_reply_to']));
...
$mailchimp_from_name = get_option('mailchimp_from_name', 'Your Newsletter Name');
$mailchimp_reply_to = get_option('mailchimp_reply_to', '');
?>
...
<div class="mailchimp-settings">
    ...
    <form method="post">
        ...
        <table class="form-table">
            ...
            <tr valign="top">
                <th scope="row"><?php esc_html_e('From Name', 'newsletter'); ?></th>
                <td>
                    <input type="text" name="mailchimp_from_name" value="<?php echo esc_attr($mailchimp_from_name); ?>" class="regular-text" required />
                    <p class="description"><?php esc_html_e('The name that appears in the "From" field of your emails.', 'newsletter'); ?></p>
                </td>
            </tr>
            <tr valign="top">
                <th scope="row"><?php esc_html_e('Reply-to Email', 'newsletter'); ?></th>
                <td>
                    <input type="email" name="mailchimp_reply_to" value="<?php echo esc_attr($mailchimp_reply_to); ?>" class="regular-text" required />
                    <p class="description"><?php esc_html_e('The email address that replies will be sent to.', 'newsletter'); ?></p>
                </td>
            </tr>
        </table>

        <?php submit_button(__('Save Mailchimp Settings', 'newsletter')); ?>
    </form>
</div>
